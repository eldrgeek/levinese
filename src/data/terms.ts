...
export const terms: Term[] = [
  ...
  },
  {
    ...
    <br><br>
    <strong>Levin's lens:</strong> In Levin's framework, calcium signals are not only metabolic triggers — they carry information across cell collectives. Calcium waves propagating through gap junctions relay error signals and stress over tissue distances, functioning as a form of cognitive glue. The embryo teratogen-resistance work (Tung et al. 2024) extends this beyond the single body: embryos exposed to a teratogen in a large cohort survive better, and with fewer defects, than embryos exposed alone. Read that result precisely — the inter-embryo assistance is mediated by short-range <em>diffusible</em> signalling involving the P2 ATP receptor, so what passes from embryo to embryo is extracellular ATP, not calcium. Calcium is the receiving end of that exchange: the intracellular transient and the intra-tissue wave that the ATP signal triggers inside each embryo. Calcium signalling is therefore one medium through which a tissue's bioelectric state becomes an inter-cellular language, routing information about anatomical error and stress from where it arises to where it can be acted on.`,
    authored_by: 'Sonnet 4.6 + Levin gloss',
    is_new: true,
